...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int val = -100000;
        int min = minOp.estimateIOCost(); // gets the minimum io cost of minOp
        List<Integer> colIndices = getEligibleIndexColumns(table);
        for (int index : colIndices) {
            SelectPredicate pred = this.selectPredicates.get(index);
            QueryOperator query = new IndexScanOperator(this.transaction, table, pred.column, pred.operator, pred.value);
            int indexIO = query.estimateIOCost(); // gets the index of the IO of the query plan
            if (min > indexIO) {
                val = index;
                minOp = query;
                min = indexIO;
        ...
        minOp = addEligibleSelections(minOp, val);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> prevSet : prevMap.keySet()) {
            for (JoinPredicate joinPred : this.joinPredicates) {
                ...
                String leftCol = joinPred.leftColumn;
                String rightCol = joinPred.rightColumn;
                QueryOperator left;
                QueryOperator right;
                HashSet keys = new HashSet<>(prevSet);
                HashSet set = new HashSet<>();

                if (prevSet.contains(leftTable) && !prevSet.contains(rightTable)) { //case 1
                    set.add(rightTable);
                    keys.add(rightTable);
                    left = pass1Map.get(set);
                    right = minCostJoinType(prevMap.get(prevSet), left, leftCol, rightCol);
                } else if (!prevSet.contains(leftTable) && prevSet.contains(rightTable)) { //case 2
                    set.add(leftTable);
                    keys.add(leftTable);
                    left = pass1Map.get(set);
                    right = minCostJoinType(prevMap.get(prevSet), left, rightCol, leftCol);
                } else { //case 3
                    continue;
                }

                if (!result.containsKey(keys)) {
                    result.put(keys, right);
                } else if (result.get(keys).estimateIOCost() > right.estimateIOCost()) {
                    result.put(keys, right);
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        ...
        Set<String> set;
        Iterator<Record> retIt;
        for (String table : tableNames) { // pass 1
            set = new HashSet<>();
            set.add(table);
            QueryOperator temp = minCostSingleAccess(table);
            map.put(set, minCostSingleAccess(table));
        }
        Map<Set<String>, QueryOperator> map2 = new HashMap<>(map);
        for (int i = 0; i < tableNames.size() - 1; i++) { // pass i
            map2 = minCostJoins(map2, map);
        }
        finalOperator = minCostOperator(map2);
        addGroupBy();
        addProject();
        addLimit();

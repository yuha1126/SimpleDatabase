...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int nodeNum = numLessThanEqual(key, getKeys());
        return getChild(nodeNum).get(key);
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int index = numLessThanEqual(key, getKeys());
        BPlusNode childNode = getChild(index);
        Optional<Pair<DataBox, Long>> output = childNode.put(key, rid); //recursive call to the child nodes

        if (!output.isPresent()) { // case 1; if the output doesn't result in the child node splitting
            return Optional.empty();
        }
        //case 2: the child node does split
        Pair<DataBox, Long> vals = output.get(); //takes out the value in Optional
        keys.add(index, vals.getFirst());
        children.add(index + 1, vals.getSecond());

        int order = metadata.getOrder();
        if (keys.size() <= order * 2) { //return empty if there's space in the current node
            sync();
            return Optional.empty();
        }
        //if there's no space, split the inner node
        List<DataBox> Lkeys = keys.subList(0, order);
        List<Long> Lchildren = children.subList(0, order + 1);
        List<DataBox> Rkeys = keys.subList(order + 1, 2*order + 1);
        List<Long> Rchildren = children.subList(order + 1, 2*order + 2);
        DataBox Mkey = keys.get(order); // doesn't need middle child because it's going to be pushed up

        InnerNode Rnode = new InnerNode(metadata, bufferManager, Rkeys, Rchildren, treeContext); //creates the right node
        Long RnodePage = Rnode.getPage().getPageNum();
        this.keys = Lkeys; //updates the left node
        this.children = Lchildren;

        sync();
        Pair<DataBox, Long> retPair = new Pair<DataBox, Long>(Mkey, RnodePage);
        return Optional.of(retPair);
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                  float fillFactor) {
        // TODO(proj2): implement
        int currIndex = getChildren().size() - 2;

        BPlusNode Rchild = getChild(currIndex + 1);
        Optional<Pair<DataBox, Long>> output = Rchild.bulkLoad(data, fillFactor);

        if (!output.isPresent()) { // if the child node doesn't get split
            return Optional.empty();
        }
        // if the child node splits
        Pair<DataBox, Long> vals = output.get();
        DataBox firstVal = vals.getFirst();
        Long secondVal = vals.getSecond();
        keys.add(currIndex + 1, firstVal);
        children.add(currIndex + 2, secondVal);
        ...
        // case if it exceeds innerNode space
        List<DataBox> Lkeys = keys.subList(0, order);
        List<Long> Lchildren = children.subList(0, order + 1);
        List<DataBox> Rkeys = keys.subList(order + 1, 2*order + 1);
        List<Long> Rchildren = children.subList(order + 1, 2*order + 2);
    ...
    @Override
    public void remove(DataBox key) { // comeback to it
        // TODO(proj2): implement
        int index = numLessThanEqual(key, getKeys());

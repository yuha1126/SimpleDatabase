...
public class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator implements Iterator<Record>{
        ...
        private Record fetchNextRecord() {
            ...
            while (true) {
                if (rightPageIterator != null && rightPageIterator.hasNext()) { // Case 1: iterating through the right source records
                    Record rightRecord = rightPageIterator.next();
                    if (compare(leftRecord, rightRecord) == 0) {
                        return leftRecord.concat(rightRecord);
                    }
                } else if (leftBlockIterator != null && leftBlockIterator.hasNext()) { // Case 2: iterating through the left Block
                    rightPageIterator.reset();
                    leftRecord = leftBlockIterator.next();

                } else if (rightSourceIterator != null && rightSourceIterator.hasNext()) { // case 3
                    fetchNextRightPage();
                    leftBlockIterator.reset();
                    leftRecord = leftBlockIterator.next();

                } else if (leftSourceIterator != null && leftSourceIterator.hasNext()) { // case 4
                    fetchNextLeftBlock();
                    rightSourceIterator.reset();
                    fetchNextRightPage();
                } else { // when we've reached the end of the records
                    return null;
                }
            }
        }

...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        if (left) {
            colIndex = getLeftColumnIndex();
        } else {
            colIndex = getRightColumnIndex();
        }

        for (Record record : records) {
            DataBox colValue = record.getValue(colIndex);
            int hash = HashFunc.hashDataBox(colValue, pass);
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        if (!probeFirst) {
            ...
            for (Record record : buildRecords) {
                DataBox joinVal = record.getValue(buildColumnIndex);
                if (!hashTable.containsKey(joinVal)) {
                    hashTable.put(joinVal, new ArrayList<>());
                }
                hashTable.get(joinVal).add(record);
            }
        }

        // Probing stage
        for (Record record: probeRecords) {
            DataBox joinVal = record.getValue(probeColumnIndex);
            if (!hashTable.containsKey(joinVal)) continue;
            // We have to join the right record with each left record with
            // a matching key
            for (Record lRecord : hashTable.get(joinVal)) {
